...
- **Source JDK path (Java 8)**: C:\Users\haiche\.jdk\jdk-8\bin
- **Target JDK path (Java 21)**: C:\Program Files\Microsoft\jdk-21.0.8.9-hotspot\bin
...
- Migrate all javax.persistence.* and javax.validation.constraints.* imports to their jakarta.* equivalents
- Keep javax.imageio.ImageIO as-is (it is part of the standard JDK, not Jakarta EE)
- Update Oracle JDBC driver from ojdbc8 to ojdbc11 for Java 21 compatibility
- Update Maven compiler source and target from 8 to 21
- Update Docker base images from eclipse-temurin:8 to eclipse-temurin:21 in Dockerfile
- Reuse the current branch for all changes
