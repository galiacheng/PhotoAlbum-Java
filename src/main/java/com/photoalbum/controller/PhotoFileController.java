...
/**
 * Controller for serving photo files from PostgreSQL binary storage
 */
@Controller
...
public class PhotoFileController {
    ...
    /**
     * Serves a photo file by ID from PostgreSQL binary storage
     */
    @GetMapping("/{id}")
    public ResponseEntity<Resource> servePhoto(@PathVariable String id) {
        ...
        try {
            ...
            // Get photo data from PostgreSQL binary storage
            ...
            logger.info("Serving photo ID {} ({}, {} bytes) from PostgreSQL",
            ...
            logger.error("Error serving photo with ID {} from PostgreSQL", id, ex);
