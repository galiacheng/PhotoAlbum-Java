    ...
    public ResponseEntity<Resource> servePhoto(@PathVariable String id) {
        ...
        try {
            ...
            if (photo.getBlobName() == null || photo.getBlobName().isEmpty()) {
                logger.error("No blob name found for photo ID {}", id);
                return ResponseEntity.notFound().build();
            }

            // Retrieve photo binary from Azure Blob Storage
            byte[] photoData = blobStorageService.downloadBlob(photo.getBlobName());
            if (photoData == null || photoData.length == 0) {
                logger.error("Empty photo data retrieved from Azure Blob Storage for photo ID {}", id);
                return ResponseEntity.notFound().build();
            }
