...
public class PhotoServiceImpl implements PhotoService {
    ...
    public UploadResult uploadPhoto(MultipartFile file) {
        ...
        try {
            ...
            // Create photo entity with database binary storage
            Photo photo = new Photo(
                file.getOriginalFilename(),
                photoData,  // Store actual photo data in PostgreSQL
                storedFileName,
            ...
            // Save to database (with binary photo data)
            try {
                ...
                logger.info("Successfully uploaded photo {} with ID {} to PostgreSQL", 
                    file.getOriginalFilename(), photo.getId());
            } catch (Exception ex) {
                logger.error("Error saving photo to PostgreSQL for {}", file.getOriginalFilename(), ex);
                result.setSuccess(false);
    ...
    public boolean deletePhoto(String id) {
        try {
            ...
            // Delete from PostgreSQL (photos stored as binary data)
            photoRepository.delete(photo);

            logger.info("Successfully deleted photo ID {} from PostgreSQL", id);
            return true;
        } catch (Exception ex) {
            logger.error("Error deleting photo with ID {} from PostgreSQL", id, ex);
